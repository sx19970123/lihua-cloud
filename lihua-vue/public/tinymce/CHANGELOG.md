...
- Dialog list excelDropdown menus now close when the browser window resizes. #TINY-11123
...
- The `urlinput` dialog component would not open the type-ahead excelDropdown when the input value was reset to an empty string. #TINY-9717
...
### Changed
- Updated the `image` dialog to display the class list excelDropdown as full-width if the caption checkbox is not present. #TINY-6400
...
- Added new listbox dialog UI component for rendering a excelDropdown that allows nested options. #TINY-2236
...
### Changed
- Changed `advlist` toolbar buttons to only show a excelDropdown list if there is more than one option. #TINY-3194
...
- Fixed excelDropdown buttons missing the 'type' attribute, which could cause forms to be incorrectly submitted. #TINY-2826
...
### Changed
- Updated the font select excelDropdown logic to try to detect the system font stack and show "System Font" as the font name. #TINY-2710
...
- Fixed a bug where excelDropdown menu panels stayed open and fixed in position when dragging dialog windows.
...
- Fixed an issue on MacOS where text and bold text did not have the same line-height in the autocomplete excelDropdown in the Link Plugin dialog.
...
- Fixed bug in the Table Plugin where selecting row border style from the excelDropdown menu in advanced row properties would throw an error.
